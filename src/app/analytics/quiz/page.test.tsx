...
const { default: QuizAnalyticsPage } = await import("./page");
...
describe("QuizAnalyticsPage", () => {
  ...
  test("shows the passcode form and no dashboard data when there is no valid session", async () => {
    ...
    const jsx = await QuizAnalyticsPage({ searchParams: noPage });
    render(jsx);
  ...
  test("paginates the participant table and clamps an out-of-range page", async () => {
    ...
    const jsx = await QuizAnalyticsPage({ searchParams: Promise.resolve({ page: "99" }) });
    render(jsx);
